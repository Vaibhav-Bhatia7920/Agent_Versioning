...
def change_line_in_file(file_path: str, line_number: int, new_content: str) -> str:
    """Change a specific line in a file to new content."""
    cmd = f"sed -i '' '{line_number}s/.*/{new_content}/' {file_path}"
    result = run_command(cmd)
    if result.exit_code != 0:
        return f"Error changing line {line_number} in file {file_path}: {result.stderr}"
    return f"Line {line_number} in file '{file_path}' has been changed to: {new_content}"

NAVIGATOR_TOOLS = [run_tree, run_ripgrep, find_files, read_file_snippet, change_line_in_file]